...
class CategoryController {
    ...
    async getCategories(req, res) {
        try {
            ...
            return res.status(500).json({
                message: 'Server error getCategories',
                error: err.message
            });
        }
    }
    ...
    async updateCategory(req, res) {
        try {
            ...
            if (!dup) return res.status(404).json({
                message: 'Category not found'
            });

            // Update the category
            const updateCategory = await CategoryModel.update(update, {
                where: {
                    idCategory: id
                }
            });
    ...
    async deleteCategory(req, res) {
        try {
            ...
            // Check if category exists
            const deleted = await CategoryModel.findOne({
                where: {
                    idCategory: id
                }
            });
            ...
            // Delete the category
            const del = await CategoryModel.destroy({
                where: {
                    idCategory: id
                }
            });
